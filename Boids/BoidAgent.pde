class BoidAgent{
 ...
 BoidAgent(float x1, float x2, float y1, float y2, float z1, float z2){
   ...
   this.position = new PVector(random(x1+10,x2-10), random(y1+10,y2-10), random(z1+10,z2-10));
 ...
 void update(float dt){
   ...
   // Boundary conditions. Appear on the other side.
   if(position.x < x1){ position.x = x1+10; velocity.x *= -1;}
   if(position.x > x2){ position.x = x2-10; velocity.x *= -1;}
   
   if(position.y < y1){ position.y = y1+10; velocity.y *= -1;}
   if(position.y > y2){ position.y = y2-10; velocity.y *= -1;}
   
   if(position.z < z1){ position.x = z1+10; velocity.z *= -1;}
   if(position.z > z2){ position.x = z2-10; velocity.z *= -1;}
 }
}
